/* Code borrowed from github repo of " C++ implementation of McCreight's algorithm construction of suffix tree"
 Author: Jingwen Ren
 */
...
treenode *jumpdown(treenode *node, node_point beta, const string &T)
{
    ...
    while (beta.pstart <= beta.pend)
    {
        ...
        { // choosing the right direction to jumpdown
            ++i;
        }
    ...
    { // jumpdown stops at node "current"
        return current;
    }
    else
    { // jumpdown stops at an edge, need split the edge and insert new node "w"
...
{ // p records that the leafnode which is going to be inserted is the p-th
  // children of the node which walkdown ends at
    ...
    while (gamma.pstart <= gamma.pend)
    {
        ...
        { // choosing the right direction to walkdown
            ++i;
        }
        if (!current->children[i])
        { // current->children[i] == NULL, indicates walkdown stops at node current
            cout << "NODE"
                 << "\t\t";
            p = i;
            return_node = current;
        ...
        else
        {
            ...
            { // this only happens when matching till node current->children[i], so continue walkdown
                current = current->children[i];
            }
            else
            { // this only happens when mismatch happens in the middle of an edge, split this edge and add a new node "new_node_v"
                cout << "EDGE"
                     << "\t\t";
...
treenode *build_suffix_tree(const string &T, int len)
{
    ...
    for (int j = 1; j <= len; ++j)
    { // now we're going to insert suffix j
        ...
        { // need jumpdown;
            ...
            if (!w->children[1])
            { // w->children[1] = NULL;  w is the split node when jumpdown is finished, so no need for walkdown, inserting leafnode directly
                ...
                cout << "EDGE"
                     ...
                     << "\t"
                     << "NA"
                     << "\t" << current_leafnode->start << "\t\t"
                     << current_leafnode->end << endl;
            }
            else
            { // jumpdown stops at an existed node w, need walkdown from node "w"
                cout << "NODE"
                     << "\t\t" << w->start << "\t" << w->end << "\t"
                     << "YES"
                     << "\t";
...
void serialize(treenode *root, FILE *fp)
{
    ...
    for (int i = 0; i < MAX_CHAR && root->children[i] != NULL; i++)
    {
...
int main(int argc, const char *const argv[])
{
    ...
    if (argc != 3)
    {
        cerr << "NEEDS INPUTS: " << argv[0] << " <FASTA-FILE> WITHOUT '$'!"
             << " AND PATH TO STORE THE TREE.TXT FILE!" << endl;
        return EXIT_FAILURE;
    }
