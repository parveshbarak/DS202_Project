...
int deserialize(treenode *node, FILE *fp)
{
    ...
    int node_end;
    char val;
    if (fscanf(fp, "%i %i ", &node_start, &node_end) == 2)
    { // successfully read two numbers instead of char "#"
        ...
        for (int i = 0; i < MAX_CHAR; ++i)
        {
            ...
            if (deserialize(node->children[i], fp))
            { // deserialize(node->children[i], fp) == 1 means no children[i] below node->children[i]
                delete children;
